...
for (( i=0; i<nbPath; i++ )); do
	...
	if [ $timing == 'PRE' ];
	then
    # qsub -pe serial 2 -e $dstDir -o $dstDir -j y $commonScriptDir/convert_vessel_and_lobe_to_mha.sh $srcDir/${pathA}.hdr $dstDir
    bash $commonScriptDir/convert_vessel_and_lobe_to_mha.sh $srcDir/${pathA}.hdr $dstDir
